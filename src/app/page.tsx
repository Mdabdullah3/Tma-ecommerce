...
import { ArrowUpRight, Handbag, ShoppingCart, Zap } from 'lucide-react';
...
export default function MobileBoutique2026() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white selection:bg-amber-500 pb-24 relative overflow-hidden">

      {/* --- MATCHED BACKGROUND IMAGE THEME --- */}
      <div className="fixed inset-0 z-0 pointer-events-none">
        {/* Deep Blue Glow (Left) */}
        <div className="absolute top-0 left-[-20%] w-[80%] h-full bg-blue-900/20 blur-[120px] rounded-full" />
        {/* Amber/Gold Glow (Right) */}
        <div className="absolute top-0 right-[-20%] w-[80%] h-full bg-amber-900/20 blur-[120px] rounded-full" />
        {/* Sub-pixel grain to make the gradient look rich */}
        <div className="absolute inset-0 opacity-[0.03] bg-[url('https://grainy-gradients.vercel.app/noise.svg')]" />
      </div>

      <SovereignHeader />

      <main className="relative z-10 pt-24 px-4 space-y-1">
        <CategorySlider />
        ...
        <div className="grid grid-cols-2 gap-4 pt-4">
          {PRODUCTS.map((item) => (
            ...
            >
              {/* --- CARD ARCHITECTURE --- */}
              <div className="relative aspect-[4/5] rounded-[40px] overflow-hidden border border-white/10 shadow-2xl transition-all duration-500 group-hover:border-amber-500/30 bg-[#0a0a0a]/40 backdrop-blur-sm">
                ...
                <div className="absolute bottom-0 inset-x-0 p-2">
                  <div className="bg-black/60 backdrop-blur-3xl border border-white/5 rounded-[28px] p-1.5 flex items-center justify-between">
                    ...
                    >
                      <div className="absolute inset-0 bg-amber-500 translate-y-12 group-hover/btn:translate-y-0 transition-transform duration-500" />
              ...
              <div className="mt-4 px-3 space-y-1">
                <div className="flex items-center gap-2">
                  <h3 className="text-xs font-black italic tracking-tighter text-white uppercase group-hover:text-amber-500 transition-colors">
                    {item.name}
                  </h3>
