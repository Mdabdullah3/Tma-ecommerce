...
import { Plus, Diamond, ShoppingBag } from 'lucide-react';
      ...
      <main className="relative z-10 pt-20 px-4">
        ...
        <div className="grid grid-cols-2 gap-x-4 mt-4 gap-y-8">
          {products.map((item, index) => {
            ...
            const glowColor = isLegendary ? '#d946ef' : '#06b6d4';
            ...
            return (
              <Link href={`/products/${item?._id}`} key={item?._id || index}>
                ...
                >
                  ...
                  />
                  <div className="relative w-full aspect-[3/4.2] rounded-[32px] overflow-hidden bg-[#050505] p-px transition-transform duration-500 group-hover:-translate-y-2 group-hover:shadow-2xl">

                    <div className="absolute inset-0 bg-linear-to-b from-white/20 via-transparent to-white/5 opacity-50" />

                    <div className="relative w-full h-full rounded-[31px] overflow-hidden bg-zinc-900">

                      {/*--- HEADER HUD --- */}
                      <div className="absolute top-0 inset-x-0 h-16 bg-linear-to-b from-black/80 to-transparent z-20 p-3 flex justify-between items-start">
                      ...
                      <div className="absolute bottom-0 inset-x-0 p-3 z-30">
                        <div className="relative bg-white/5 backdrop-blur-2xl border-t border-white/10 rounded-[24px] p-3 overflow-hidden group-hover:bg-white/10 transition-colors duration-500">
                          ...
                          <div className="flex justify-between items-end gap-2">
                            ...
                            `}
                            >
                              {!isInCart && <div className="absolute inset-0 bg-linear-to-br from-white via-zinc-100 to-zinc-400 opacity-50" />}
                              {isInCart ? (
                                <ShoppingBag
                                  size={22}
                                  strokeWidth={2.5}
